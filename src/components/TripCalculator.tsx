...
const initialExpenses: ExpenseItem[] = [
  { id: "flights", icon: Plane, label: "Flights", value: 0, color: "bg-secondary" },
  { id: "hotels", icon: Hotel, label: "Hotels", value: 0, color: "bg-primary" },
  { id: "cruises", icon: Ship, label: "Cruises", value: 0, color: "bg-gradient-ocean" },
  { id: "tickets", icon: Ticket, label: "Tickets", value: 0, color: "bg-accent" },
  { id: "activities", icon: Sparkles, label: "Activities", value: 0, color: "bg-success" },
];
...
export const TripCalculator = () => {
  ...
  const updateExpense = (id: string, delta: number) => {
    setExpenses((prev) =>
      prev.map((exp) =>
        exp.id === id
          ? { ...exp, value: Math.max(0, Math.ceil((exp.value + delta) / 50) * 50) }
          : exp
      )
    );
  };
  ...
  const setExpenseValue = (id: string, value: number) => {
    setExpenses((prev) =>
      prev.map((exp) =>
        exp.id === id ? { ...exp, value: Math.max(0, Math.ceil(value / 50) * 50) } : exp
      )
    );
  };
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <h2 className="font-display text-4xl md:text-5xl font-bold mb-4">
            Trip Cost <span className="text-gradient-sunset">Calculator</span>
          </h2>
              ...
              <Card className="p-6 bg-card shadow-card hover:shadow-hover transition-all duration-300 border-0">
                ...
                <div className="flex items-center justify-between gap-4">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    onClick={() => updateExpense(expense.id, -100)}
                  ...
                  <input
                    type="number"
                    value={expense.value}
                    onChange={(e) => setExpenseValue(expense.id, parseInt(e.target.value) || 0)}
                    className="flex-1 text-center text-2xl font-display font-bold bg-transparent border-b-2 border-border focus:border-primary outline-none py-2"
                  />
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    onClick={() => updateExpense(expense.id, 100)}
                    className="w-10 h-10 rounded-full bg-muted hover:bg-success hover:text-success-foreground transition-colors flex items-center justify-center"
                  >
